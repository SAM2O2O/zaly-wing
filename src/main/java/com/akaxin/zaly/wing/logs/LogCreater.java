...
public class LogCreater {
	...
	private static final String DEFAULT_SITE_PATH = "logs/";
	...
	public static Logger createLogger(String logName, String logPath, PatternLayout layout, boolean additivity,
			boolean isDailyMode) {
		Logger createdNewLogger = Logger.getLogger(logName);
		try {
			...
			createdNewLogger = Logger.getLogger(logName);
			createdNewLogger.removeAllAppenders();
			createdNewLogger.setAdditivity(additivity);
			createdNewLogger.addAppender(fileAppender);
		} catch (Exception e) {
			logger.error("create logger error", e);
			throw new RuntimeException();
		}
		return createdNewLogger;
	}
}
