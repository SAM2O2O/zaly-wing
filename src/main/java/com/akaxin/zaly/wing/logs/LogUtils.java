 ...
 */
package com.akaxin.zaly.wing.logs;
...
public class LogUtils extends LogCreater {
	...
	public static void requestInfoLog(Logger logger, Command command, String messagePattern, Object... objects) {
		...
		logger.info("client={} siteUserId={} action={} msg={}", command.getClientIp(), command.getAction(),
				command.toString(), format.getMessage());
		return;
	}

	public static void requestDebugLog(Logger logger, Command command, String requestStr) {
		logger.debug("client={} action={} command={} request={}", command.getClientIp(), command.getAction(),
				command.toString(), requestStr);
	}

	public static void requestErrorLog(Logger logger, Command command, Throwable t) {
		logger.error(StringHelper.format("client={} action={} uri={} error", command.getClientIp(), command.getAction(),
				command.getUri()), t);
	}

	public static void requestErrorLog(Logger logger, Command command, Class<?> clazz, Throwable t) {
		logger.error(StringHelper.format("client={} action={} uri={} {} error", command.getClientIp(),
				command.getAction(), command.getUri(), clazz.getClass().getName()), t);
	}

	public static void requestResultLog(Logger logger, Command command, CommandResponse response) {
		try {
			logger.info("client={} action={} uri={} cost={}ms result={}", command.getClientIp(), command.getAction(),
