...
import com.akaxin.zaly.wing.client.ZalysClientTest;
 ...
 */
public class ZalysClientHandler extends SimpleChannelInboundHandler<RedisCommand> {
	private static final Logger logger = LoggerFactory.getLogger(ZalysClientHandler.class);

	private final ZalysClientTest zalysClient;

	public ZalysClientHandler(ZalysClientTest zalysClient) {
		this.zalysClient = zalysClient;
	}
	...
	protected void channelRead0(ChannelHandlerContext ctx, RedisCommand redisCmd) {
		...
		try {
			...
			this.zalysClient.handleResponse(new RedisCommandResponse(errInfo, result));
